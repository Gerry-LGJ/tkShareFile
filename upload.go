...
func Upload(conf *Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !(len(uploadDir) - strings.LastIndexByte(uploadDir, '/') == 1) { uploadDir += "/" }//如果在分享目录的根目录，就不加"/"
		...
		c.JSON(http.StatusOK, gin.H{"status": 0, "msg": filename})
	}
}
