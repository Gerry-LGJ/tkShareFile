...
func Index(conf *Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		playerBtnCount := 0	// 记录生成"想播放？"按钮的个数，然后采用轮询的方式定位点击的按钮的兄弟标签<a>的href
		for _, v := range list {
			name := v.Name()
			//li += "<li><a href=\"/share" + url + "/" + name + "\" style=\"color: #fff;\">" + name + "</a></li>\n"
			switch CheckFileType(name) {
			case ".mp3":
				li += "<li><a href=\"/share" + url + "/" + name + "\" style=\"color: #fff;\">" + name +
					"</a><button class=\"play_btn\" onclick=\"player(" + fmt.Sprintf("%d", playerBtnCount) + ")\">想播放？</button></li>\n"
				playerBtnCount++
			case ".mp4":
				li += "<li><a href=\"/share" + url + "/" + name + "\" style=\"color: #fff;\">" + name +
					"</a><button class=\"play_btn\" onclick=\"player(" + fmt.Sprintf("%d", playerBtnCount) + ")\">想播放？</button></li>\n"
...
func Player(conf *Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		if !conf.Player { panic("您没有试听权限，请联系管理员。")}								// 检查是否开放试听
...
// 可能需要转码，因为有一些文件名很奇怪
func ExtractFromEnd(str string, char byte) (string, error) {
	sLen := len(str)
	for i, _ := range str {
		if str[sLen - i - 1] == '/' { return str[sLen - i:], nil } }
	return "", errors.New(fmt.Sprintf("search for '%v' failed", char))
}
